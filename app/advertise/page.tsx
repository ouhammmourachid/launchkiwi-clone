        ...
        <div id="booking-form" className="max-w-4xl mx-auto rounded-xl border border-white/10 bg-[#14170f] p-6 sm:p-8">
          {submitted ? (
            <div className="py-12 text-center space-y-4">
              <div className="mx-auto w-12 h-12 rounded-full bg-[#d97706]/20 flex items-center justify-center text-[#d97706]">
                <CheckIcon />
              </div>
              <h3 className="text-2xl font-bold text-[#f3efe6]">Booking Submitted!</h3>
              <p className="text-sm text-[#9ca38d] max-w-md mx-auto">
                Thank you! We have received your booking request for the {selectedDuration}-day spotlight.
                Check your inbox at <span className="text-[#f3efe6] font-medium">{email || "your email"}</span> for payment instructions and launch details.
              </p>
              <button
                onClick={() => setSubmitted(false)}
                className="mt-4 text-xs text-[#d97706] underline hover:text-[#f3efe6]"
              >
                Book another ad slot
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                className="w-full rounded-lg bg-[#c26d15] hover:bg-[#d97706] py-3 text-sm font-bold text-[#f3efe6] transition-colors shadow-lg disabled:opacity-50"
              >
                {isSubmitting
                  ? "Processing..."
                  : `Book ${selectedDuration} days — $${selectedDuration === "30" ? "30" : "79"} \u2192`}
              </button>
